...
import {InlineRules , Breaks, GfmIR, Pedantic } from './inlineRules'
...
export class InlineLexer {
  ...
  constructor(links, options) {
    ...
    this.links = links;
    this.rules = InlineRules.getInstance().getInline();
    ...
    if (this.options.gfm) {
      if (this.options.breaks) {
        this.rules = Breaks.getInstance().getInline();
      } else {
        this.rules = GfmIR.getInstance().getInline();
      }
    } else if (this.options.pedantic) {
      this.rules = Pedantic.getInstance().getInline();
    }
  }
  ...
  public output(src) {
    ...
    while (src) {
      ...
      if (cap) {
        src = src.substring(cap[0].length);
        out += this.renderer.codespan(Helpers.escape(cap[2], true));
        continue;
      }
